...
-- 3_2. 테이블 스페이스 할당해줘야됨! (SAMPLE 계정을 변경)
...
-- 5. SAMPLE계정에게 KH.EMPLOYEE테이블에 조회(SELECT)할 수 있는 권한 부여하기
GRANT SELECT ON KH.EMPLOYEE TO SAMPLE;

-- 6. SAMPLE계정에게 KH.DEPARTMENT테이블에 삽입(INSERT)할 수 있는 권한 부여하기
...
/*
    < 롤 ROLE >
    특정 권한들을 하나의 집하브로 모아놓은 것

    CONNECT  : CREATE SESSION (데이터베이스에 접속할 수 있는권한)
    RESOURCE : CREATE TABLE, CREATE SEQUENCE, ... (특정 객체들을 새성할 수 있는 권한)
*/
...
-- 일반 사용자 계정 새서 후 CONNECT, RESOURCE 정도의 최소한의 권한만 부여한거였음
