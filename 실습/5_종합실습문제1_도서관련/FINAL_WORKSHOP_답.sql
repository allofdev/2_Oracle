...
-- 3. 도서명이 25자 이상인 책 번호와 도서명을 화면에 출력하는 SQL구문을 작성하시오.
SELECT BOOK_NO
     , BOOK_NM
  FROM TB_BOOK
...
--    가장 먼저 표시되는 작가 이름과 사무실 전화번호, 집 전화번호, 휴대폰 전화번호를 표시하는 SQL구문을 작성하시오
SELECT * FROM TB_WRITER;
...
--    많이 쓴 순서대로 표시하는 SQL구문을 작성하시오.
--    단, 동명이인인 작가는 없다고 가정한다. 
...
--     제시된 내용에 맞게 "TB_BOOK_TRASLATOR" 테이블을 생성하는 SQL 구문을 작성하시오.
--     (PRIMARY KEY 제약 조건 이름은 "PK_BOOK_TRANSLATOR"로 하고, 
--      REFERENCE 제약 조건 이름은 "FK_BOOK_TRANSLATOR_01", "FK_BOOK_TRANSLATOR_02"로 할 것_
...
--     도서 저자 정보 테이블에서 도서 역자 정보 테이블(TB_BOOK_TRANSLATOR)로 옮기는 SQL구문을 작성하시오.
...
-- 12. 2007년도에 출판된 번역서 이름과 번역자(역자)를 표시하는 SQL구문을 작성하시오.
...
-- 13. 12번 결과를 활용하여 대상 번역서들의 출판일을 변경할 수 없도록 하는 뷰를 생성하는 SQL구무을 작성하시오.
--     (뷰 이름은 "VW_BOOK_TRANSLATOR"로 하고 도서명, 번역자, 출판일이 표시되도록 할것)
...
--     해당 컬럼이 NULL인 경우 '지음'으로 변경하는 SQL 구문을 작성하시오 (COMMIT 처리 할 것)
...
--     사무실이 서울이고, 사무실 전화번호 국번이 3자리인 작가의 이름과
--     사무실 전화번호를 표시하는 SQL구문을 작성하시오
SELECT * FROM TB_WRITER;
...
-- 18. 2006년 1월 기준으로 등록된 지 31년 이상 된 작가 이름
SELECT * 
  FROM TB_WRITER 
...
--     '황금가지' 출판사에서 발행한 도서 중 재고 수량이 10권 미만인 
--     도서명과 가격, 재고상태를 표시하는 SQL구문을 작성하시오.
...
--     재고 수량이 90권 이상인 도서에 대해 도서명, 재고수량, 
--     원래 가격, 20%인하 가격을 표시하는 SQL구문을 작성하시오.
--     (결과 헤더는 도서명, 재고수량, 가격(Org), 가격(New)로 표시할 것.
--     재고 수량이 많은순, 할인 가격이 높은 순, 도서명 순으로 표시되도록 할 것)
SELECT BOOK_NM "도서명"
     , STOCK_QTY "재고수량"
     , PRICE "가격(Org)"
     , PRICE * 0.8 "가격(New)"
  FROM TB_BOOK
 WHERE STOCK_QTY >= 90 
   AND MONTHS_BETWEEN(SYSDATE, ISSUE_DATE) >= 360
 ORDER BY 2 DESC
        , 4 DESC
        , 1 ASC;
